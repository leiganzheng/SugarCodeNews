...
- (void)setupWebView {
    ...
    [HttpTool get:@"http://www.sugarcode.net/?api=post" params:@{@"id":self.ID} success:^(id json) {
        [SVProgressHUD dismiss];
        NSString *contentStr = [json objectAtPath:@"data/post_content"];
        [webView loadHTMLString:contentStr baseURL:[NSURL URLWithString:@"http://www.sugarcode.net"]];

    } failure:^(NSError *error) {
        
    }];
    
   // [webView loadHTMLString:self.contentStr baseURL:[NSURL URLWithString:@"http://www.sugarcode.net"]];
