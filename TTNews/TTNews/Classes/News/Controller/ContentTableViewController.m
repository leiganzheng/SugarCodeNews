...
-(void)fetchNewHeaderNews {
    [self.headerView removeTimer];
    [HttpTool get:@"http://www.sugarcode.net/?api=posts" params:@{@"page":[NSNumber numberWithInteger:self.currentPage],@"cid":self.channelName} success:^(id json) {
                [SVProgressHUD dismiss];
        NSError *error;
      self.headerNewsArray =  [TTHeaderNews arrayOfModelsFromDictionaries:[json objectAtPath:@"data/list"] error:&error];
        
               // self.headerNewsArray = array;
                NSMutableArray *imageUrls = [NSMutableArray array];
                NSMutableArray *titles = [NSMutableArray array];
                for (TTHeaderNews *news in self.headerNewsArray){
                    [imageUrls addObject:news.img_url];
                    [titles addObject:news.post_title];
                }
...
-(void)fetchNewNormalNews {
    ...
    [HttpTool get:@"http://www.sugarcode.net/?api=posts" params:@{@"page":[NSNumber numberWithInteger:self.currentPage],@"cid":self.channelName} success:^(id json) {
        [SVProgressHUD dismiss];
        NSError *error;
        self.normalNewsArray =  [TTHeaderNews arrayOfModelsFromDictionaries:[json objectAtPath:@"data/list"] error:&error];

        
        NSMutableArray *imageUrls = [NSMutableArray array];
        NSMutableArray *titles = [NSMutableArray array];
        for (TTHeaderNews *news in self.normalNewsArray){
...
-(void)loadMoreData {
    [SVProgressHUD show];
    TTHeaderNews *news = self.normalNewsArray.lastObject;
...
//    }
    [HttpTool get:@"http://www.sugarcode.net/?api=posts" params:@{@"page":[NSNumber numberWithInteger:self.currentPage],@"cid":self.channelName} success:^(id json) {
        ...
        [self.normalNewsArray addObjectsFromArray:[TTHeaderNews arrayOfModelsFromDictionaries:[json objectAtPath:@"data/list"]]];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
        TTHeaderNews *news = self.normalNewsArray[indexPath.row];
...
//    } else {
        [self pushToDetailViewControllerWithUrl:news.ID];
   // }
}
...
-(void)pushToDetailViewControllerWithUrl:(NSString *)contentStr {
    DetailViewController *viewController = [[DetailViewController alloc] init];
   // viewController.url = contentStr;
  //  viewController.contentStr = contentStr;
    viewController.ID = contentStr;
    [self.navigationController pushViewController:viewController animated:YES];
}
