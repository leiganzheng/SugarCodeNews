    ...
    if (!_currentChannelsArray) {
       ...
       // NSArray *array = [[NSUserDefaults standardUserDefaults] arrayForKey:@"currentChannelsArray"];
        ...
        if (_currentChannelsArray.count == 0) {
            [HttpTool get:@"http://www.sugarcode.net/?api=categories" params:nil success:^(id json) {
              //  [json objectAtPath:@"info/value"];
                NSError *error;
                NSArray *array = [CategorieModel arrayOfModelsFromDictionaries:[json objectAtPath:@"data/list"] error:&error];
                [_currentChannelsArray removeAllObjects];
                for (CategorieModel *mo in array) {
                    [_currentChannelsArray addObject:mo.cat_name ];
